...
public class PlacementValidTest {
    ...
    public void testInvalidPiece() {
        Random r = new Random();
        for (int i = 0; i < TestUtility.BASE_ITERATIONS; i++) {
    ...
    public void testGoodPairs() {
        for (int i = 0; i < TestUtility.GOOD_PAIRS.length; i++) {
            String test = TestUtility.GOOD_PAIRS[i];
    ...
    public void testBadPairs() {
        for (int i = 0; i < TestUtility.BAD_PAIRS.length; i++) {
            String test = TestUtility.BAD_PAIRS[i];
            assertFalse("Placement '" + test + "' is invalid, but passed.", LinkGame.isPlacementValid(test));
        }
    }
    ...
    public void testGood() {
        for (int i = 0; i < TestUtility.PLACEMENTS.length; i++) {
            String p = TestUtility.shufflePlacement(TestUtility.PLACEMENTS[i]);
    ...
    public void testDoubleUp() {
        Random r = new Random();
        for (int i = 0; i < TestUtility.PLACEMENTS.length; i++) {
            String p = TestUtility.PLACEMENTS[i];
